...
const SearchManifacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
            ...
            <Combobox.Options>
              {(
                filteredManufacturers.map((item)=>(
                  <Combobox.Option value={item} key={item} className={({active})=>`relative search-manufacturer_option ${active ? 'bg-primary-blue text-white ' : 'text-gray-900'} `} >
                    {item}
                  </Combobox.Option>
                ))
                )}
